...
enum animationMode { IMMOBILE_MODE, TORUS_MODE, TEAPOT_MODE, K_MODE, SPHERE_MODE, CONE_MODE, CUBE_MODE };
...
/* K */
float r_x, r_y, move_r_x, move_r_y;
...
void display(void) // Here's Where We Do All The Drawing
{
	...
	glRotated(-90.0f, 2.0f, 0.0f, 0.0f);
	if (mode == CONE_MODE) {
		if (high_cone >= 0.55)
			grow_cone = -0.0005;
	...
	if (mode == TORUS_MODE) {
		...
		else if (inner_torus <= 0.025) {
			grow_torus = 0.0001;
		}
		inner_torus += grow_torus;
		outer_torus -= grow_torus;
	}
	...
	glMaterialfv(GL_FRONT, GL_SHININESS, no_shininess);
	glMaterialfv(GL_FRONT, GL_EMISSION, no_mat);
	glutSolidTorus(inner_torus, outer_torus, 50, 50);
	glPopMatrix();
	...
	glVertex3f(0.0 + r_x, -0.2, 0.0);
	glVertex3f(-0.1 + r_x, -0.2, 0.0);
	glVertex3f(-0.1 + r_x, 0.2, 0.0);
	glVertex3f(0.0 + r_x, 0.2, 0.0);
	glVertex3f(0.0 + r_x, -0.1, 0.0);
	glVertex3f(0.125 + r_x, -0.1, 0.0);
	glVertex3f(0.175 + r_x, -0.2, 0.0);
